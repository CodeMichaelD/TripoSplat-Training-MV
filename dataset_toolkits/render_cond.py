...
def _render_cond(file_path, sha256, output_dir, num_views, engine, output_folder, new_light=False, verbose=False, unit_radius=False, p_ortho=0.1):
    ...
    ortho_scale = []
    for r in radius:
        if np.random.rand() > p_ortho:
            if unit_radius:
                fov.append(2 * np.arcsin(1.0 / 2 / r))
            else:
                fov.append(2 * np.arcsin(np.sqrt(3) / 2 / r))
            ortho_scale.append(None)
        else:
            fov.append(None)
            if unit_radius:
                ortho_scale.append(np.random.uniform(np.sqrt(2) / np.sqrt(3), 1.0))
            else:
                ortho_scale.append(np.random.uniform(np.sqrt(2), np.sqrt(3)))
        
    views = [{'yaw': y, 'pitch': p, 'radius': r, 'fov': f, 'ortho': o is not None, 'ortho_scale': o} for y, p, r, f, o in zip(yaws, pitchs, radius, fov, ortho_scale)]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--p_ortho', type=float, default=0.1)
    ...
    opt = edict(vars(opt))
    output_folder = "verbose/renders_cond" if opt.verbose else "renders_cond"
    ...
    if opt.instances is None:
        ...
        if 'cond_rendered' in metadata.columns and not opt.verbose:
    ...
    # process objects
    func = partial(_render_cond, output_dir=opt.output_dir, num_views=opt.num_views, engine=opt.engine, output_folder=output_folder, new_light=opt.new_light, verbose=opt.verbose, unit_radius=opt.unit_radius, p_ortho=opt.p_ortho)
