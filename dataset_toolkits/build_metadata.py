...
if __name__ == '__main__':
    ...
    # detect models
    image_models = []
    if os.path.exists(os.path.join(opt.output_dir, 'pcd_features')):
        image_models = os.listdir(os.path.join(opt.output_dir, 'pcd_features'))
    latent_models = []
    if os.path.exists(os.path.join(opt.output_dir, 'latents')):
        latent_models = os.listdir(os.path.join(opt.output_dir, 'latents'))
    print(f'Point-projected image models: {image_models}')
    print(f'Latent models: {latent_models}')

    if 'captions' not in metadata.columns:
        metadata['captions'] = [None] * len(metadata)
    if 'aesthetic_score' not in metadata.columns:
        metadata['aesthetic_score'] = [None] * len(metadata)
    ...
    for model in image_models:
        if f'pcd_feature_{model}' not in metadata.columns:
            metadata[f'pcd_feature_{model}'] = [False] * len(metadata)
    ...
    # merge point-projected features
    for model in image_models:
        df_files = [f for f in os.listdir(opt.output_dir) if f.startswith(f'pcd_feature_{model}_') and f.endswith('.csv')]
        df_parts = []
        for f in df_files:
            try:
                df_parts.append(pd.read_csv(os.path.join(opt.output_dir, f)))
        ...
        with ThreadPoolExecutor(max_workers=os.cpu_count()) as executor, \
            ...
            def worker(sha256):
                try:
                    ...
                    for model in image_models:
                        if need_process(f'pcd_feature_{model}') and \
                            metadata.loc[sha256, f'pcd_feature_{model}'] == False and \
                            metadata.loc[sha256, 'rendered'] == True and \
                            os.path.exists(os.path.join(opt.output_dir, 'pcd_features', model, f'{sha256}.npz')):
                            metadata.loc[sha256, f'pcd_feature_{model}'] = True
                    for model in latent_models:
    ...
    with open(os.path.join(opt.output_dir, 'statistics.txt'), 'w') as f:
        ...
        if len(image_models) != 0:
            f.write(f'  - Number of assets with point-projected image features:\n')
            for model in image_models:
                f.write(f'    - {model}: {metadata[f"pcd_feature_{model}"].sum()}\n')
