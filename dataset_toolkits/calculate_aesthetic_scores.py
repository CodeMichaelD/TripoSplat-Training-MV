...
if __name__ == "__main__":
    ...
    if 'rendered' in metadata.columns:
        metadata = metadata[metadata['rendered'] == 1]
    ...
    with ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
        ...
        def load_image(sha256):
            try:
                render_dir = os.path.join(opt.output_dir, 'renders', sha256, 'image')
                ...
                processed = []
                for file in files:
                    image = Image.open(os.path.join(render_dir, file))
            ...
            if processed is not None:
                with torch.no_grad():
                    ...
                    rows.append(pd.DataFrame({
                        ...
                        'aesthetic_score': [aesthetic_score.max().item()],
                    }))
