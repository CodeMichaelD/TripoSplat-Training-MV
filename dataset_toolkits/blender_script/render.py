...
def init_render(engine='CYCLES', resolution=512, geo_mode=False):
    ...
    bpy.context.scene.render.image_settings.quality = 100  # lossless max webp quality
...
def scene_bbox() -> Tuple[Vector, Vector]:
    ...
    scene_meshes = [obj for obj in bpy.context.view_layer.objects if isinstance(obj.data, bpy.types.Mesh)]

    for obj in scene_meshes:
        if obj.hide_viewport or obj.hide_render:
            continue
            
        # Check if object is in "glTF_not_exported" collection
        in_not_exported = False
        for col in obj.users_collection:
            if col.name == "glTF_not_exported":
                in_not_exported = True
                break
        ...
        if n_verts > 0:
            ...
            mesh_eval.vertices.foreach_get("co", verts)
            ...
            bbox_min = np.minimum(bbox_min, np.min(verts_world, axis=0))
            bbox_max = np.maximum(bbox_max, np.max(verts_world, axis=0))
...
def normalize_scene(unit_radius) -> Tuple[float, Vector]:
    ...
    if unit_radius:
        scale = 1 / (bbox_max - bbox_min).length
    else:
    ...
    return [list(bbox_min), list(bbox_max)], scale, offset
...
def main(arg):
    ...
    if arg.object.endswith(".blend"):
        ...
        print(f"[INFO] Loaded object {arg.object}")
        if arg.split_normal:
            split_mesh_normal()
    ...
    # normalize scene
    aabb, scale, offset = normalize_scene(arg.unit_radius)
    ...
    for name, output in outputs.items():
        to_export[f"{name}_frames"] = []
        
    views = json.loads(arg.views)
    for i, view in enumerate(views):
        cam_ortho = view['ortho']
        if cam_ortho:
        ...
        for name, output in outputs.items():
            output.base_path = os.path.join(args.output_folder, name)
            output.file_slots[0].path = f'{i:03d}'
        bpy.context.scene.render.filepath = os.path.join(arg.output_folder, 'image', f'{i:03d}.webp')
        ...
        for name, output in outputs.items():
            ...
            try:
                os.rename(f'{output.base_path}/{i:03d}0001.{ext}', os.path.join(arg.output_folder, name, f'{i:03d}.{ext}'))
            except Exception as e:
                fns = os.listdir(output.base_path)
                for fn in fns:
                    if fn.split('/')[-1].startswith(f'{i:03d}') and len(fn.split('/')[-1].split('.')[0])==7:
                        fns = [fn]
                        break
                    fn = None
                os.rename(os.path.join(output.base_path, fn), os.path.join(arg.output_folder, name, f'{i:03d}.{ext}'))
            to_export[f"{name}_frames"].append({
                "file_path": f'{name}/{i:03d}.{ext}',
                "camera_angle_x": fov,
                "camera_ortho_scale": ortho_scale,
                "camera_orthographic": cam_ortho,
                "transform_matrix": get_transform_matrix(cam)
    ...
    if arg.save_mesh:
        ...
        # export ply mesh
        bpy.ops.wm.ply_export(filepath=os.path.join(arg.output_folder, 'mesh.ply'), forward_axis='Y', up_axis='Z', apply_modifiers=True, export_selected_objects=False, export_uv=False, export_normals=False, export_colors='NONE', export_triangulated_mesh=True)
