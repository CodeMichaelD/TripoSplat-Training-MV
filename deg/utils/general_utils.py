...
def get_noise_like(x: Union[torch.Tensor, TensorDict]):
    if isinstance(x, torch.Tensor):
        return torch.randn_like(x)
    elif isinstance(x, TensorDict):
        return x.apply(torch.randn_like)
    elif isinstance(x, dict):
        return {k: get_noise_like(v) for k, v in x.items()}
    else:
        raise NotImplementedError
    
def get_zeros_like(x: Union[torch.Tensor, TensorDict]):
    if isinstance(x, torch.Tensor):
        return torch.zeros_like(x)
    elif isinstance(x, TensorDict):
        return x.apply(torch.zeros_like)
    elif isinstance(x, dict):
        return {k: get_zeros_like(v) for k, v in x.items()}
    else:
        raise NotImplementedError
     
def terms_merge(*terms_list):
    """
    Add existing terms in terms_list.
    """
    terms = edict()
    for terms_ in terms_list:
        for k, v in terms_.items():
            if k in terms:
                terms[k] = terms[k] + v
            else:
                terms[k] = v
...
def play_beeps(k=3, interval=0.5):
    """
    Play beeps.
    """
    import time
